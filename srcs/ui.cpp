...
namespace Ui {
    void MainWindow::setupUi(QMainWindow* mainWindow) {
        ...
        if (QFileInfo::exists(iconPath))
            ...
            QMessageBox::warning(nullptr, "警告", "图标文件不存在！");
        ...
        common_show->setStyleSheet("border: 1px solid gray; font-size: 30px;");
        common_show->setFixedSize(350, 80);
        lcd_show = new QLCDNumber();
        lcd_show->setDigitCount(8);
        lcd_show->setSegmentStyle(QLCDNumber::Flat);
        lcd_show->display("--------");
        lcd_show->setStyleSheet("border: 1px solid gray; color: red; background-color: black;");
